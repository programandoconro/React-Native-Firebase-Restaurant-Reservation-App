...
function parseMessage(data, binary) {
  if (binary) {
    _logger.default.error('Expected text message, got binary!');
  ...
  try {
    ...
    _logger.default.error(`Received message had wrong protocol version: ${message.version}`);
  } catch (e) {
    _logger.default.error(`Failed to parse the message as JSON:\n${data}`);
  }
    ...
    for (const [otherId, otherWs] of clients) {
      if (otherId !== broadcasterId) {
        try {
          ...
          _logger.default.error(`Failed to send broadcast to client: '${otherId}' ` + `due to:\n ${e.toString()}`);
        }
      }
    }
  }
  ...
  wss.on('connection', clientWs => {
    ...
    function handleCaughtError(message, error) {
      ...
      if (message.id === undefined) {
        _logger.default.error(`Handling message from ${clientId} failed with:\n${error}\n` + `message:\n${JSON.stringify(errorMessage)}`);
      } else {
        try {
          ...
          _logger.default.error(`Failed to reply to ${clientId} with error:\n${error}` + `\nmessage:\n${JSON.stringify(errorMessage)}` + `\ndue to error: ${e.toString()}`);
        }
      }
    }
    ...
    clientWs.onmessage = event => {
      ...
      if (message === undefined) {
        _logger.default.error('Received message not matching protocol');
